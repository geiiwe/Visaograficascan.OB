...
const FastAnalysisIndicators: React.FC<FastAnalysisIndicatorsProps> = ({ results }) => {
  ...
  const volatilityIndicators = results.filter(r => 
    r.type === "volatility" || r.type === "candleSize" || r.description.includes("volatilidade"));
  
  // Is the market considered volatile?
  const isVolatileMarket = volatilityIndicators.some(r => r.strength > 70);
  
  // Identify volume indicators
  const volumeIndicators = results.filter(r => 
    r.type === "volumeSpikes" || r.type === "volume" || r.description.includes("volume"));
  
  // Is there significant volume?
  const hasSignificantVolume = volumeIndicators.some(r => r.strength > 65);
  ...
  const getIndicatorIcon = (type: string) => {
    switch (type) {
      ...
      case "volumeSpikes": 
      case "volume": return <Volume className="h-3.5 w-3.5 mr-1.5" />;
      case "candleFormation": return <Zap className="h-3.5 w-3.5 mr-1.5" />;
      case "candleSize": return <ChartCandlestick className="h-3.5 w-3.5 mr-1.5" />;
      ...
      case "volatility": return <CircleDashed className="h-3.5 w-3.5 mr-1.5" />;
  ...
  return (
    <div className="absolute top-26 right-2 z-30">
      ...
      {results.length > 1 && (
        <div className={cn(
          "flex flex-col rounded-lg px-3 py-2 text-xs text-white font-medium shadow-lg backdrop-blur-sm mb-2 border",
          directionSummary === "up" 
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex gap-1">
              ...
              {hasManipulationRisk && (
                <div className="bg-yellow-500/80 text-[10px] px-1.5 py-0.5 rounded-full flex items-center">
              ...
              {hasStrongConsensus && !hasManipulationRisk && (
                <div className="bg-blue-500/80 text-[10px] px-1.5 py-0.5 rounded-full">
                  Consenso
                </div>
              )}
            </div>
          ...
          <div className="mt-2 flex gap-2 items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px]">
                Volatilidade: {isVolatileMarket ? "Alta" : "Normal"}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px]">
                Volume: {hasSignificantVolume ? "Forte" : "Moderado"}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Individual Indicators sorted by strength with category grouping */}
      <div className="flex flex-col gap-2 mt-2">
        {/* Price Action & Candlestick Indicators */}
        {results
          .filter(r => ["priceAction", "candleFormation", "candleSize"].includes(r.type))
          .sort((a, b) => b.strength - a.strength)
          .map((result, index) => (
            <div 
              key={`price-${result.type}-${index}`}
              className={cn(
        ...
        {results
          .filter(r => ["volumeSpikes", "volume", "momentum"].includes(r.type))
          .sort((a, b) => b.strength - a.strength)
          .map((result, index) => (
            <div 
              key={`volume-${result.type}-${index}`}
              className={cn(
                "flex items-center justify-between rounded-full px-3 py-1.5 text-xs text-white shadow-lg backdrop-blur-sm", 
                result.direction === "up" ? "bg-trader-green/70" : 
                result.direction === "down" ? "bg-trader-red/70" : 
        ...
        {results
          .filter(r => ["rsiAnalysis", "macdCrossover", "bollingerBands", "volatility"].includes(r.type))
          .sort((a, b) => b.strength - a.strength)
          .map((result, index) => (
            <div 
              key={`tech-${result.type}-${index}`}
              className={cn(
                "flex items-center justify-between rounded-full px-3 py-1.5 text-xs text-white shadow-lg backdrop-blur-sm", 
                result.direction === "up" ? "bg-trader-green/70" : 
                result.direction === "down" ? "bg-trader-red/70" : 
        ...
        {/* Warning Indicators (OTC patterns, etc) */}
        {results
          .filter(r => ["otcPatterns", "priceReversal"].includes(r.type))
          .sort((a, b) => b.strength - a.strength)
          .map((result, index) => (
            <div 
              key={`warning-${result.type}-${index}`}
              className={cn(
                "flex items-center justify-between rounded-full px-3 py-1.5 text-xs text-white shadow-lg backdrop-blur-sm", 
                "bg-yellow-600/70 border border-yellow-400/30",
                result.strength > 85 ? "border-yellow-400/70" : ""
              )}
              style={{ marginTop: index > 0 ? '0.5rem' : '0' }}
            >
              <div className="flex items-center">
